...
class App {
  ...
  applyStoreToUI() {
    ...
    // Adicionar evento change para simWalletSelect
    const simWalletSelect = document.getElementById('simWalletSelect')
    if (simWalletSelect) {
      simWalletSelect.addEventListener('change', e => {
        store.updateInput('simWalletSelect', e.target.value)
        this.runCalculation()
      })
    }

    // Adicionar evento change para enableTier3
    const enableTier3 = document.getElementById('enableTier3')
    if (enableTier3) {
      enableTier3.addEventListener('change', e => {
        const isChecked = e.target.checked
        store.updateInput('enableTier3', String(isChecked))

        // Mostrar/ocultar campos do Nível 2 e Nível 3
        const tier2Range = document.getElementById('tier2Range')
        const tier2Default = document.getElementById('tier2Default')
        const tier3Container = document.getElementById('tier3Container')

        if (isChecked) {
          tier2Range?.classList.remove('hidden')
          tier2Default?.classList.add('hidden')
          tier3Container?.classList.remove('hidden')
        } else {
          tier2Range?.classList.add('hidden')
          tier2Default?.classList.remove('hidden')
          tier3Container?.classList.add('hidden')
        }
      ...
      const isTier3Enabled = inputs.enableTier3 === 'true'
      enableTier3.checked = isTier3Enabled
  ...
  toggleFuturePlanning() {
    ...
    if (newVal) {
      ...
      // Capturar valor do campo de seleção de carteira
      const simWalletSelect = document.getElementById('simWalletSelect')
      if (simWalletSelect) {
        store.updateInput('simWalletSelect', simWalletSelect.value)
      }
    }
