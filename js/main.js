...
class App {
  ...
  createOrientationButton() {
    // Criar botão flutuante para controle de orientação
    const button = document.createElement('button')
    button.id = 'orientationToggleBtn'
    button.className =
      'fixed bottom-4 left-4 w-12 h-12 bg-blue-600 hover:bg-blue-500 text-white rounded-full shadow-lg flex items-center justify-center transition-all duration-300 z-50 md:hidden'
    button.innerHTML = '<i class="fas fa-mobile-alt text-sm"></i>'
    button.title = 'Alterar orientação da tela'

    // Adicionar evento de clique para alternar orientação
  ...
  toggleOrientation() {
    ...
    const isLandscape = body.classList.contains('landscape-mode')
    ...
    if (isLandscape) {
      ...
      if (button) {
        button.innerHTML = '<i class="fas fa-mobile-alt text-sm"></i>'
        button.title = 'Mudar para paisagem'
      }
      // Tentar remover orientação landscape se possível
      ...
      if (button) {
        ...
        button.title = 'Mudar para retrato'
      }
      // Tentar forçar orientação landscape se possível
      if (screen.orientation && screen.orientation.lock) {
        screen.orientation.lock('landscape').catch(() => {
          // Ignorar erro se não for possível bloquear orientação
        })
      }
    }
  }
  ...
  addInvestment() {
    ...
    // Adicionar ajuste manual para registrar a saída do dinheiro da carteira
    if (wallet !== 'none') {
      const walletLabel =
        wallet === 'personal' ? 'Carteira Pessoal' : 'Carteira de Receita'
      const adjustment = {
        date: date,
        amount: -val, // Valor negativo para indicar saída
        wallet: wallet,
        description: `Aplicação em investimento: ${name}`
      }

      // Adicionar aos ajustes existentes ou criar nova lista
      const existingAdjustments = store.state.adjustments || []
      store.setState({
  ...
  togglePortfolioDetails() {
    ...
    // Ajustar altura do container baseado no estado
    if (container) {
      if (isHidden) {
        // Está abrindo - aumentar altura
        container.classList.remove('min-h-[100px]')
        container.classList.add('min-h-[300px]')
      } else {
        // Está fechando - reduzir altura
...
>>>>>>> C:/Users/Patrick Godoy/.windsurf/worktrees/Gestor-SP/Gestor-SP-184f9fbc/js/main.js
