...
export const Renderer = {
  ...
  renderTodayClosing(dailyData) {
    ...
    const adjTotal = (data.inAdjustmentPersonal || 0) + (data.inAdjustmentRevenue || 0)
    if (adjTotal !== 0) {
        const isPositive = adjTotal > 0
        const colorClass = isPositive ? 'text-emerald-400' : 'text-red-400'
        const sign = isPositive ? '+' : '-'
        ...
        transactionItems.push(`
            <div class="flex justify-between items-center text-[10px] text-slate-300 bg-slate-900/40 p-2 rounded-lg border border-slate-700/30 hover:bg-slate-700/40 transition-colors">
                <div class="flex items-center gap-2">
                    <i class="fas fa-sliders-h text-orange-400 w-3 text-center"></i>
                    <span class="font-medium">Ajuste Manual</span>
                </div>
                <span class="font-bold ${colorClass} font-mono">${sign}${Formatter.currency(Math.abs(adjTotal))}</span>
            </div>
        `)
    }
